...
export class ZeroAPI {
  ...
  private captureRouteDoc(method: string, path: string, handlers: any[]): void {
    if (!this.swaggerDocs || !this.swaggerDocs.isEnabled()) return;
    ...
    for (const handler of handlers) {
      if (handler._docs) {
        ...
        for (const [handlerName, docs] of handler._docs) {
          this.swaggerDocs.documentRoute(method, path, docs);
        }
      }
    }
  }
  ...
  private setupSwaggerRoutes(): void {
    if (!this.swaggerDocs) return;

    const basePath = this.swaggerDocs.getBasePath();

    // Serve OpenAPI JSON specification
    this.get(`${basePath}/json`, (req: any, res: any) => {
      const spec = this.swaggerDocs!.generateSpec();
      res.json(spec);
    });

    // Serve Swagger UI
    this.get(`${basePath}`, (req: any, res: any) => {
      const html = this.swaggerDocs!.generateUI();
